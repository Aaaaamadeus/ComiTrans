...
2. **日文 OCR**：ONNX Manga-OCR 识别日文；检测为英文、或 OCR 结果明显是英文乱码的文本块会整块保留，不 OCR、不擦除、不嵌字。
...
当前 OCR 是日文模型，识别英文会乱码。程序会在两层识别英文：检测器直接标出的英文块，以及 OCR 结果中拉丁/全角拉丁占比 ≥ 40% 的乱码块。命中后整块跳过 OCR、擦除和嵌字，英文原文保留在图上。如需翻译英文，请开启多模态翻译，或等待后续加入英文 OCR。
